...
def init_db():
    with db_connect() as conn:
        ...
        mevcut_kolonlar = {row['name'] for row in conn.execute('PRAGMA table_info(projeler)').fetchall()}
        ...
        proje_sayisi = conn.execute('SELECT COUNT(*) as toplam FROM projeler').fetchone()['toplam']
        if proje_sayisi == 0:
            conn.execute(
...
def get_projects():
    with db_connect() as conn:
        rows = conn.execute('SELECT * FROM projeler ORDER BY COALESCE(yil, 0) DESC, id DESC').fetchall()
...
def takimimiz():
    with db_connect() as conn:
        rows = conn.execute('SELECT * FROM uyeler ORDER BY departman, id').fetchall()
    uyeler = [dict(r) for r in rows]
...
def uye_guncelle(uid):
    ...
    with db_connect() as conn:
        mevcut = conn.execute('SELECT foto FROM uyeler WHERE id=?', (uid,)).fetchone()
        if not mevcut:
            return jsonify({'ok': False, 'error': 'Bulunamadı'}), 404
        foto_adi = mevcut['foto']
        foto_dosya = request.files.get('foto')
        if foto_dosya and foto_dosya.filename and allowed_file(foto_dosya.filename):
            ext = foto_dosya.filename.rsplit('.', 1)[1].lower()
            foto_adi = uuid.uuid4().hex + '.' + ext
...
def admin():
    ...
    projeler = get_projects()
    with db_connect() as conn:
        mesajlar_rows = conn.execute('SELECT * FROM iletisim_mesajlari ORDER BY id DESC').fetchall()
    mesajlar = [dict(r) for r in mesajlar_rows]

    return render_template('admin.html', projeler=projeler, mesajlar=mesajlar)


# Proje Silme Rotası — sadece POST ile çalışır (CSRF korumalı)
...
def duzenle(id):
    ...
    if request.method == 'POST':
        ...
        with db_connect() as conn:
            mevcut = conn.execute('SELECT resim FROM projeler WHERE id = ?', (id,)).fetchone()
            if not mevcut:
                return redirect(url_for('admin'))

            resim_adi = mevcut['resim']
